...
export default function Header () {
  ...
  <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
    <a href="/" className="flex items-center">
        <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">kunalznk</span>
    </a>
    ...
    <div className="hidden w-full md:block md:w-auto" id="navbar-default">
      <ul className="font-medium text-ternary flex flex-col p-4 md:p-0 mt-4 border  rounded-lg md:flex-row md:space-x-8 md:mt-0 md:border-0">
        {
          ["Home", "Work", "Blog", "Skills" ,"Contact"].map((label, index) => {
            return <>
            <li key={index} className="group" onClick={( ) => setSelected(index)}>
            <a href={`#${label}`} className="uppercase block py-2 pl-3 pr-4 text-white - rounded md:bg-transparent" >{label}</a>
            <div className={`w-full bg-white h-[2px] rounded-lg
              transform opacity-0 transition-all duration-300 ease-in
              -translate-x-20 
              ${selected === index ? "translate-x-0 opacity-100" : ""} 
            `}  />
            </li>
            </>
          })
